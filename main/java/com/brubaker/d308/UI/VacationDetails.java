...
public class VacationDetails extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Excursion list click handling
        excursionListAdapter.setOnExcursionClickListener(excursion -> {
            Intent intent = new Intent(VacationDetails.this, ExcursionDetails.class);
            intent.putExtra("excursionId", excursion.getExcursionId());
            intent.putExtra("vacationId", vacationId); // Pass the associated vacation ID
        ...
        fab.setOnClickListener(v -> {
            if (vacationId == -1) {
                Toast.makeText(this, "Please save the vacation before adding excursions.", Toast.LENGTH_SHORT).show();
                return;
            }
            Intent intent = new Intent(VacationDetails.this, ExcursionDetails.class);
            intent.putExtra("vacationId", vacationId); // Pass the vacationId to ExcursionDetails
            startActivity(intent);
        });
...
//        fab.setOnClickListener(v -> {
//            Intent intent = new Intent(VacationDetails.this, ExcursionDetails.class);
//            intent.putExtra("vacationId", vacationId); // Passing the ID of the vacation
//            startActivity(intent);
//        });
    }
    ...
    protected void onResume() {
        super.onResume();
        if (vacationId != -1) {
            loadExcursions(); // Reload excursions whenever the activity is resumed
        }
    }
    ...
    private void saveVacation() {
        ...
        Log.d("VacationDetails", "Before saving vacationId: " + vacationId);

        Vacation vacation;
        if (vacationId == -1) { // New vacation
            vacation = new Vacation(title, hotel, startDate, endDate); // Let Room auto-generate the ID
            vacationId = repository.insertVacation(vacation); // Retrieve the generated ID
            Log.d("VacationDetails", "After saving vacationId: " + vacationId);
            Toast.makeText(this, "New vacation saved.", Toast.LENGTH_SHORT).show();
        } else { // Update existing vacation
            vacation = new Vacation(vacationId, title, hotel, startDate, endDate); // Use existing ID
    ...
    private void deleteVacation() {
        repository.getAllExcursionsForVacation(vacationId).observe(this, excursions -> {
            if (excursions != null && !excursions.isEmpty()) {
                Log.d("VacationDetails", "Cannot delete vacation with ID: " + vacationId + ", excursions still exist.");
                Toast.makeText(this, "Cannot delete a vacation with associated excursions. Please delete excursions first.", Toast.LENGTH_SHORT).show();
            } else {
                Log.d("VacationDetails", "Deleting vacation with ID: " + vacationId);
...
//    }
    ...
    private void shareVacation() {
        ...
        if (title.isEmpty() || hotel.isEmpty() || startDateText.isEmpty() || endDateText.isEmpty()) {
            Toast.makeText(this, "Please fill all fields before sharing vacation details.", Toast.LENGTH_SHORT).show();
            return;
        }

        // Format the vacation details
        ...
        // Create a share chooser with clipboard option
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Share Vacation Details")
                .setItems(new String[]{"Share via Apps", "Copy to Clipboard"}, (dialog, which) -> {
                    if (which == 0) {
                        // Option 1: Share via apps
    ...
    private void copyToClipboard(String text) {
        ...
        ClipData clip = ClipData.newPlainText("Vacation Details", text);
